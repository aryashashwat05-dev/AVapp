...
import { collection, query, doc, addDoc, serverTimestamp, writeBatch, getDocs } from "firebase/firestore";
...
export default function RoomPage() {
  ...
  const { data: roomData, isLoading: isRoomLoading } = useDocument(roomRef);
  ...
  const { data: userProfile, isLoading: isProfileLoading } = useDocument(userProfileRef);

  const hasVoted = !!userVote;
  const isAdmin = roomData?.ownerId === user?.uid && userProfile?.role === 'admin';
  ...
  const handleVote = async (optionName: string) => {
    ...
    if (hasVoted) {
        toast({ variant: 'destructive', title: 'Already Voted', description: 'You can only vote once.' });
        return;
    }
  ...
  const handleResetVoting = async () => {
    ...
    try {
        ...
        toast({ title: "Voting Reset", description: "All votes have been archived and the session is reset." });
    } catch (error: any) {
  ...
  if (isUserLoading || isRoomLoading || areVotesLoading || isUserVoteLoading || isProfileLoading) {
    return (
            ...
            <div className="lg:col-span-1 space-y-8">
             {!isAdmin && (
                <Card>
                    ...
                    <CardDescription>
                        {hasVoted ? "You have already voted." : "Your vote is anonymous and final."}
                    </CardDescription>
                    </CardHeader>
             ...
             {isAdmin && (
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Manage the voting session and room details.</CardDescription>
                    </CardHeader>
                        ...
                        <AlertDialog>
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                        This will archive all current votes and reset the session. This action cannot be undone.
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
